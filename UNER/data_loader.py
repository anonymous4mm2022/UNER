...
class InputExample(object):
    def __init__(self, guid, img_id, words, labels,domain_words):
        self.guid = guid  # int
        self.img_id = img_id  # int
        self.words = words  # list
        self.labels = labels  # list
        self.domain_words = domain_words #list
...
class InputFeatures(object):
    ...
    def __init__(self, word_ids, char_ids, img_feature,dns_feature, mask, label_ids,token_ids,token_length,domain_token_ids,domain_token_length):
        self.word_ids = word_ids
        self.char_ids = char_ids
        self.img_feature = img_feature
        self.dns_feature = dns_feature
        self.mask = mask
        self.label_ids = label_ids
        self.token_ids = token_ids
        self.token_length = token_length
        self.domain_token_ids = domain_token_ids
        self.domain_token_length = domain_token_length
...
class WebsiteProcessor(object):
    ...
    def load_img_features(self):
        ...
        #return joblib.load(os.path.join(self.args.data_dir, self.args.img_feature_file))
    def load_dns_features(self):
        return torch.load(os.path.join(self.args.data_dir, self.args.dns_feature_file))
        #return joblib.load(os.path.join(self.args.data_dir, self.args.dns_feature_file))
    ...
    def _create_examples(self, sentences, set_type):
        ...
        # add domain
        import pandas as pd 
        df = pd.read_csv(os.path.join(self.args.data_dir, 'domain.csv'))


        for (i, sentence) in enumerate(sentences):
            words, labels, img_id = sentence[0], sentence[1], sentence[2]
            df_ = df.loc[df['id']=='IMGID:{}'.format(img_id)]
            domains = list(df_['domain'])
            domain = domains[0]
            domain_words = domain.strip().split('.')
            ...
            if i % 10000 == 0:
                logger.info(sentence)
            examples.append(InputExample(guid=guid, img_id=img_id, words=words, labels=labels,domain_words=domain_words))
...
def convert_examples_to_features(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # 1.1 Load img feature
        try:
            img_feature = img_features[example.img_id]
        except:
            logger.warning("Cannot load image feature! (IMGID: {})".format(example.img_id))
            continue
        # 1.2 Load dns feature
        try:
            dns_feature = dns_features[example.img_id]
        except Exception as e:
            logger.warning("Cannot load dns feature! (IMGID: {})".format(example.img_id))
            continue
        ...
        if len(words)<max_seq_len:
            for i in range(len(words),max_seq_len):
                words.append(tokenizer.unk_token)
        ...
        for i in range(len(token_ids),max_token_length):
            token_ids.append(tokenizer.unk_token_id)

        domain_words = example.domain_words.copy()
        if len(domain_words)<max_seq_len_dns:
            for i in range(len(domain_words),max_seq_len_dns):
                domain_words.append(tokenizer.unk_token)
        domain_words = domain_words[:max_seq_len_dns] 
        text = ' '.join(domain_words)

        domain_token_ids = tokenizer.encode(text, add_special_tokens=True)
        #print(token_ids)
        max_token_length = 512
        domain_token_length = len(domain_token_ids)
        for i in range(len(domain_token_ids),max_token_length):
            domain_token_ids.append(tokenizer.unk_token_id)
        ...
        if ex_index < 5:
            ...
            logger.info("token_length: %s" % token_length)
            logger.info("domain_token_ids: %s" % " ".join([str(x) for x in domain_token_ids]))
            logger.info("domain_token_length: %s" % domain_token_length)
        ...
        features.append(
            InputFeatures(word_ids=word_ids,
                          ...
                          domain_token_length = domain_token_length
                          )
            )
...
def load_data(args, mode):
    processor = WebsiteProcessor(args)
    ...
    features = convert_examples_to_features(examples = examples,
                                            img_features = processor.load_img_features(),
                                            dns_features = processor.load_dns_features(), 
                                            max_seq_len = args.max_seq_len,
                                            max_seq_len_dns = args.max_seq_len_dns,
    ...
    all_word_ids = torch.tensor([f.word_ids for f in features], dtype=torch.long).to(device)
    all_char_ids = torch.tensor([f.char_ids for f in features], dtype=torch.long).to(device)
    all_token_ids = torch.tensor([f.token_ids for f in features],dtype=torch.long).to(device)
    all_token_length = torch.tensor([f.token_length for f in features],dtype=torch.long).to(device)
    all_domain_token_ids = torch.tensor([f.domain_token_ids for f in features],dtype=torch.long).to(device)
    all_domain_token_length = torch.tensor([f.domain_token_length for f in features],dtype=torch.long).to(device)
    all_img_feature = torch.stack([f.img_feature for f in features]).to(device)
    all_dns_feature = torch.stack([f.dns_feature for f in features]).to(device)
    #print(all_img_feature.shape,all_dns_feature.shape)
    #for f in features:
    #    print(f.dns_feature.shape)
    all_mask = torch.tensor([f.mask for f in features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in features], dtype=torch.long)
    #all_texts = torch.tensor([f.texts for f in features])
    ...
    dataset = TensorDataset(all_word_ids, 
                            all_char_ids, 
                            all_img_feature,
                            all_dns_feature, 
                            all_mask, 
                            ...
                            all_token_ids,
                            all_token_length,
                            all_domain_token_ids,
                            all_domain_token_length
                            )
    #device = "cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu"
    return dataset
