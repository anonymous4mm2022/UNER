...
import os
import shutil
from trainer import Trainer, OPTIMIZER_LIST,Trainer1,Trainer2,Trainer3
...
def main(args):
    ...
    base_dev_file = args.dev_file
    base_test_file = args.test_file
    base_model_file = args.model_file
    results = {}
    for i in range(args.k_fold):
        args_ = args
        args_.train_file = '{}{}'.format(base_train_file,i)
        args_.dev_file = '{}{}'.format(base_dev_file,i)
        args_.test_file = '{}{}'.format(base_test_file,i)
        args_.model_file = '{}{}'.format(base_model_file,i)
        build_vocab(args_)
        train_dataset = load_data(args_, mode="train")
        dev_dataset = load_data(args_, mode="dev")
        test_dataset = load_data(args_, mode="test")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--do_cross", action="store_true", help="Whether to cross train the model")
